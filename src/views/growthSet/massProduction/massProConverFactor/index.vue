...
<script setup lang="ts" name="massProConverFactor">
...
import {
	getMasscoeffApi,
	postMasscoeffUpdateApi,
	postMasscoeffExportApi,
	postMasscoeffImportExcelApi,
	postMasscoeffSetBaseProjectApi,
} from '/@/api/growthSet/massProConverFactor';
// 定义变量内容
const { t } = useI18n();
...
const getTableData = async () => {
	...
	const res = await getMasscoeffApi(data);
...
const submitUpload = async (formEl: EmptyObjectType | undefined) => {
	const res = await postMasscoeffImportExcelApi(formEl!.raw);
	if (res.status) {
...
const onExportTable = async () => {
	...
	const res = await postMasscoeffExportApi(data);
...
const setCurrent = (scope: EmptyObjectType) => {
	ElMessageBox.confirm(`是否選擇 <span style="color:red">${scope.row.projectName}</span> 此專案為基準機種？`, t('message.hint.tips'), {
		...
		.then(async () => {
			const res = await postMasscoeffSetBaseProjectApi({ ...scope.row, baseflag: 'Y' });
			getTableData();
		})
		.catch(() => {});
};
...
const addData = async (ruleForm: EmptyObjectType, type: string) => {
	...
	const res = await postMasscoeffUpdateApi(ruleForm);
	if (res.status) {
