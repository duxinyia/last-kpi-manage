...
<script setup lang="ts" name="netOutputSet">
...
import {
	getOutputinfoByPageApi,
	postOutputinfoUpdateApi,
	postOutputinfoDeleteApi,
	postOutputinfoExportApi,
	postOutputinfoImportExcelApi,
} from '/@/api/growthSet/netOutputSet';
// 定义变量内容
const { t } = useI18n();
...
const getTableData = async () => {
	...
	const res = await getOutputinfoByPageApi(data);
	...
	res.data[res.data.length - 1].delIsShow = res.data[res.data.length - 1].editIsShow = true;
};
// 觸底
...
const downloadTemp = () => {
	window.open(
		`${import.meta.env.MODE === 'development' ? import.meta.env.VITE_API_URL : window.webConfig.webApiBaseUrl}/doc/Outputinfo.xlsx`,
		'_blank'
	);
};
// 上传文件
const submitUpload = async (formEl: EmptyObjectType | undefined) => {
	const res = await postOutputinfoImportExcelApi(formEl!.raw);
	if (res.status) {
...
const onExportTable = async () => {
	...
	const res = await postOutputinfoExportApi(data);
...
const setCurrent = (scope: EmptyObjectType) => {
	ElMessageBox.confirm(t('是否選擇此專案為基準機種？'), t('message.hint.tips'), {
...
const addData = async (ruleForm: EmptyObjectType, type: string) => {
	...
	const res = await postOutputinfoUpdateApi({ ...ruleForm, yyyy: ruleForm.yandm });
	if (res.status) {
...
const onTableDelRow = async (row: EmptyObjectType, type: string) => {
	const res = await postOutputinfoDeleteApi({ ...row, yyyy: row.yandm });
	if (res.status) {
