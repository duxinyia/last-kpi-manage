...
<script setup lang="ts" name="stageAssessReport">
...
import {
	getExamsetstageApi,
	getCalculateExamsetstageApi,
	postReportinfoSaveApi,
	getPreviewExamsetstageApi,
} from '/@/api/departmenAssessment/stageAssessReport';
// 引入组件
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'lastyearsalary', colWidth: '', title: '上年度階段薪資', type: 'text', isCheck: true },
			{ key: 'growthratePer', colWidth: '', title: '同比標準成長率', type: 'text', isCheck: true },
			{ key: 'standardsalary', colWidth: '', title: '本年度階段標準薪資', type: 'text', isCheck: true },
			{ key: 'actsalary', colWidth: '', title: '本年度階段實際薪資', type: 'text', isCheck: true },
			{ key: 'savepropPer', colWidth: '', title: '節省比例', type: 'text', isCheck: true },
			{ key: 'savesalary', colWidth: '', title: '節省金額', type: 'text', isCheck: true },
			{ key: 'savereward', colWidth: '', title: '獎勵額度', type: 'text', isCheck: true },
		],
		// 配置项（必传）
		config: {
			total: 0, // 列表总数
...
const tData = (datas: EmptyArrayType) => {
	...
	datas.forEach((item) => {
		...
		item.savepropPer = item.saveprop != null ? item.saveprop + '%' : '';
...
const getTableData = async () => {
	...
	const res = await getExamsetstageApi(data);
...
const onOpentopBtnOther = async (_: any, type: string) => {
	if (type === 'com') {
		state.tableData.config.loading = true;
		const res = await getCalculateExamsetstageApi({ YearStr: state.tableData.form.YearStr });
		let datas = tData(res.data);
		state.tableData.data = datas;
		if (res.status) {
			state.tableData.config.loading = false;
		...
		ElMessageBox.confirm(`確定保存嗎？`, t('message.hint.tips'), {
			...
			.then(async () => {
				...
				const res = await postReportinfoSaveApi({ runId: '', yandm: fromData.YearStr, rtype: 'Stage', rvalue: JSON.stringify(state.tableData.data) });
				// getTableData();
