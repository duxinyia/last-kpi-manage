...
<script setup lang="ts" name="costShifting">
import { defineAsyncComponent, reactive, ref, onMounted } from 'vue';
import { ElMessage } from 'element-plus';
import { useI18n } from 'vue-i18n';
import { getDeptInfoGetKPIOrganizeApi } from '/@/api/global';
import { getCostShiftApi, postCostShiftUpdateApi, postCostShiftExportApi, postCostShiftImportExcelApi } from '/@/api/payrollExpense/costShifting';
// 引入组件
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'costShift', colWidth: '', title: '合計', type: 'text', isCheck: true },
...
const downloadTemp = () => {
	window.open(
		`${import.meta.env.MODE === 'development' ? import.meta.env.VITE_API_URL : window.webConfig.webApiBaseUrl}/doc/CostShift.xlsx`,
		'_blank'
	);
};
// 上传文件
const submitUpload = async (formEl: EmptyObjectType | undefined) => {
	const res = await postCostShiftImportExcelApi(formEl!.raw);
	if (res.status) {
...
const onExportTable = async () => {
	...
	const res = await postCostShiftExportApi(data);
...
const getTableData = async () => {
	...
	const res = await getCostShiftApi(data);
...
const addData = async (ruleForm: EmptyObjectType, type: string) => {
	...
	try {
		const res = await postCostShiftUpdateApi(ruleForm);
		if (res.status) {
